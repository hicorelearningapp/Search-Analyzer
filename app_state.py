...
class AppStateManager:
    ...
    @classmethod
    def get_app_state(cls) -> 'AppStateManager':
        return cls()
    ...
    def get_or_create_session(self, session_id: Optional[str]) -> Dict[str, Any]:
        """Always return a valid session, creating one if needed."""
        if not session_id or session_id not in self.sessions:
            return self.create_session("auto")
        return {"session_id": session_id, "created_at": self.sessions[session_id].created_at}
